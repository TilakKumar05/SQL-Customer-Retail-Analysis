...
--Q1 Determined the total number of rows in each of the three tables in the database.
...
--Q2 Calculated the total number of transactions that include a return.
Select count(Qty) as Transaction_having_returns
From Transactions
where Qty < 0

--Q3 Converted all date variables in the datasets into valid date formats for accurate analysis.
update Transactions
set tran_date = CONVERT(date,tran_date,103)

--Q4 Identified the time range of the transaction data and present it in terms of days, months, and years in separate columns.
...
--Q5 Found the product category to which the sub-category "DIY" belongs.
...
--Data Analysis
--Q1 Identify the transaction channel that is used most frequently.
...
--Q2 Number of male and female customers in the database
Select Gender, count(customer_id) as [Count]
From Customer
Where Gender = 'M' or Gender = 'F'
group by Gender

--Q3 The city with the maximum number of customers and the total count of customers from that city.
...
--Q4 Total number of sub-categories under the "Books" category.
...
--Q5 Maximum quantity of products ever ordered in a single transaction.
...
--Q6 Net total revenue generated by the "Electronics" and "Books" categories.
...
--Q7 Number of customers with more than 10 transactions, excluding returns.
...
--Q8 Combined revenue earned from the "Electronics" and "Clothing" categories through "Flagship stores."
...
--Q9 Total revenue generated from "Male" customers in the "Electronics" category, displaying total revenue by product sub-category.
...
--Q10 Percentage of sales and returns by product sub-category, displaying only the top 5 sub-categories in terms of sales.
...
--Q11 Net total revenue generated by customers aged 25 to 35 years during the last 30 days of transactions from the maximum transaction date available in the data.
Select total_amt, 
 ...
 --Q12 Product category with the highest value of returns in the last three months of transactions.
...
--Q13 Store type that sells the maximum products, both by sales amount and by quantity sold.
Select Top 1 store_type , sum(Qty) as Qty_sold, Sum(total_amt) as Sales_amt
From Transactions
Group by Store_type
Order by sum(Qty) desc

--Q14 Product categories with average revenue above the overall average revenue.
...
--Q15 Average and total revenue by sub-category for the top 5 categories in terms of quantity sold.
